...
Método de decisão: rodar todos no **conjunto de validação anotado** (≥ 30 fragmentos: granito claro/escuro, mármore com veios, quartzito, superfícies polidas e brutas). Eliminar, nesta ordem, IoU da pior classe ≤ 0,95, Hausdorff máximo > 2 mm, licença não aprovada e pesos/revisão não verificados; nunca decidir pela média entre classes. Entre os qualificados, escolher o automático de menor custo local e, separadamente, o backend promptable de menor custo. A latência é a mediana das primeiras 20 medições registradas. Se uma das duas funções ficar sem candidato qualificado, D1 permanece aberta. Registrar a decisão no doc 01.
